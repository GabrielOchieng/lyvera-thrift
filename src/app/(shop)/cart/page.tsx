...
export default function CartPage() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [step, setStep] = useState(1);
  const [manualMode, setManualMode] = useState(false); // New state to trigger the manual UI
  const [formData, setFormData] = useState({
    name: "",
    phone: "",
    location: "",
  ...
  const handleSubmitOrder = async () => {
    setIsSubmitting(true);
    try {
      // 1. Create the Order in your DB (important for tracking)
      const result = await createOrder({
        name: formData.name,
        phone: formData.phone,
        location: formData.location,
        transCode: `MANUAL_${crypto.randomUUID().substring(0, 8)}`,
        items: cart,
        total: total,
      });

      if (result.success) {
        // 2. BYPASS STK PUSH: We skip the API call and go straight to manual instructions
        setManualMode(true);
      } else {
        alert(result.error);
      }
  ...
  return (
    <main className="min-h-screen bg-zinc-50">
      <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto">
        {/* Header & Cart Items Logic (Same as your original) */}
        <div className="flex items-center gap-4 mb-10">
        ...
        {cart.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-4">
              {/* Cart Item Mapping (Keep your existing mapping here) */}
              {cart.map((item) => (
                <div
                  key={item.id}
                ...
                >
                  ...
                  <button
                    onClick={() => removeFromCart(item.id)}
                    className="p-4 text-zinc-300 hover:text-red-500"
                  >
                    <Trash2 className="h-5 w-5" />
                  </button>
                </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-8 rounded-[1rem] border border-zinc-200 shadow-xl sticky top-24">
                ...
                <button
                  onClick={() => setIsModalOpen(true)}
                  disabled={hasSoldOutItems}
                  className="w-full cursor-pointer py-3 rounded-2xl font-black uppercase bg-maroon-primary text-white"
                >
                  Checkout
                </button>
              </div>
          ...
          <div className="text-center py-32">Empty Bag</div>
        )}
      </div>

      {/* --- CHECKOUT MODAL --- */}
      {isModalOpen && (
        <div className="fixed inset-0 z-100 bg-black/60 backdrop-blur-md flex items-center justify-center p-2">
          <div className="bg-white w-full max-w-md rounded-[2rem] overflow-hidden shadow-2xl">
            <div className="bg-maroon-primary p-4 text-white text-center">
            ...
            <div className="p-6">
              {manualMode ? (
                ...
                <div className="space-y-4 text-center animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <button
                    ...
                    className="w-full cursor-pointer bg-maroon-primary text-white py-4 rounded-xl font-black uppercase text-lg shadow-lg"
                  >
                    Confirm Payment
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <input
                    ...
                    className="w-full bg-zinc-50 rounded-2xl p-4 outline-none border-2 border-transparent focus:border-maroon-primary"
                  />
                  <input
                  ...
                  <button
                    onClick={handleNextStep}
                    className="w-full cursor-pointer bg-maroon-primary text-white py-5 rounded-2xl font-black uppercase mt-4"
                  >
                    Next: Payment
                  </button>
                </div>
              ) : (
                /* --- STEP 2: SUMMARY BEFORE MANUAL TRIGGER --- */
                <div className="space-y-6 text-center">
                  <div className="bg-zinc-50 p-6 rounded-2xl border border-dashed border-zinc-200">
                    ...
                    <p className="text-3xl font-black text-maroon-primary leading-none">
                      KES {total.toLocaleString()}
                    </p>
                  </div>
                  <p className="text-sm text-zinc-500 px-4">
                    Click below to get payment instructions for your order.
                  </p>
                  <button
                    onClick={handleSubmitOrder}
                    disabled={isSubmitting}
                    className="w-full cursor-pointer bg-maroon-primary text-white py-5 rounded-2xl font-black uppercase shadow-lg flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? (
              ...
              <button
                onClick={() => setIsModalOpen(false)}
                className="w-full text-zinc-400 text-[10px] font-bold uppercase tracking-widest mt-6"
              >
                Cancel
              </button>
            </div>
