...
export default function CartPage() {
  ...
  // 1. Availability Polling: Checks if items are sold every 5 seconds
  useEffect(() => {
    setMounted(true);
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (result.success) {
        clearCart();
        router.push("/cart/success"); // Redirect to the new success page
      } else {
        alert(result.error);
        setIsModalOpen(false); // Close modal so they can see which item sold out
      }
    } catch (error) {
      alert("An unexpected error occurred. Please try again.");
    } finally {
            ...
            <div className="lg:col-span-2 space-y-4">
              {hasSoldOutItems && (
                <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-center gap-3 text-red-600 mb-6">
                  ...
                  <p className="text-xs font-bold uppercase tracking-tight">
                    Someone just bought an item in your cart. Remove it to
                    proceed.
                  </p>
                </div>
              )}
              ...
              {cart.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`bg-white p-4 rounded-4xl border flex items-center gap-6 shadow-sm transition-all relative overflow-hidden ${
                      isSold ? "border-red-200 opacity-60" : "border-zinc-200"
                    }`}
                  >
                    {isSold && (
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-8 rounded-[2.5rem] border border-zinc-200 shadow-xl sticky top-24">
                ...
                <button
                  ...
                  className={`w-full py-5 rounded-2xl font-black uppercase italic tracking-tighter transition-all shadow-lg active:scale-95 ${
                    hasSoldOutItems
