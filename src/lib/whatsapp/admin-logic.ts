...
export async function processAdminImage(message: any) {
  ...
  // 2. Download Media URL & Image Buffer
  const mediaResponse = await fetch(
    `https://graph.facebook.com/v21.0/${imageId}`,
    { headers: { Authorization: `Bearer ${process.env.WHATSAPP_TOKEN}` } },
  );
  ...
  // 4. GROUPING LOGIC (The Fix for Duplicate Listings)
  // Look for a product created in the last 45 seconds.
  const gracePeriod = new Date(Date.now() - 45000);

  // If this message has NO caption, it is almost certainly a follow-up image.
  // We search for the most recently created product to attach it to.
  if (!caption) {
    const recentProduct = await prisma.product.findFirst({
      where: { createdAt: { gte: gracePeriod } },
      orderBy: { createdAt: "desc" },
    });

    if (recentProduct) {
      const uploadResponse = await uploadPromise;
      console.log(`📎 Appending follow-up image to: ${recentProduct.name}`);
      return await prisma.product.update({
        where: { id: recentProduct.id },
        data: { images: { push: uploadResponse.secure_url } },
      });
    }
  }

  // 5. NEW PRODUCT FLOW (Only triggered if there's a caption or no recent product)
  const extractDataWithAI = async () => {
    const models = [
      "gemini-1.5-flash",
      "gemini-1.5-pro",
      ...
      try {
        ...
        const result = await model.generateContent([
          ...
          `TASK: Catalog this product.
           ADMIN INPUT: "${caption}"
           EXTRACTION RULES:
           1. Size and Price come from the Input.
           2. Generate a catchy name, category, and 1-sentence description.
           Respond ONLY with JSON: 
